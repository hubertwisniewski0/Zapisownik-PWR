...
void LoginWindow::PolwroGUI()
{
    ...
    //     QString czasNowego =
    //     info.modified().toTimeSpec(Qt::LocalTime).toString("dd.MM.yyyy
    //     hh.mm"); QString pytanie = (QString("Na serwerze znajduje się plik z
    //     dnia %2.\nMożesz go pobrać i odciążyć serwery!"))
    //       .arg(czasNowego);

    //     buttons->clear();
    //     buttons->disconnect();
    //     buttons->addButton(QDialogButtonBox::No)->setText("Pobierz z
    //     polwro"); buttons->addButton(QDialogButtonBox::Yes)->setText("Pobierz
    //     z zew. serwera"); buttons->setEnabled(true);
...
void LoginWindow::PolwroPobieranie()
{
    ...
    QString nazwy[9] = {"Matematycy",  "Fizycy",      "Chemicy",
                        "Elektronicy", "Językowcy",   "Humaniści",
                        "Sportowcy",   "Informatycy", "Inni"};
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        progressBarBot->setValue(0);
        QString link = "https://polwro.pl/viewforum.php?f=" + typ[i] +
                       "&topicdays=0&start=0";
        reply = get_page(link);
        ...
        {
            QString link = "https://polwro.pl/viewforum.php?f=" + typ[i] +
                           "&topicdays=0&start=" + QString::number(j);
            reply = get_page(link);
            QByteArray page = reply->readAll();
            QTextStream stream(&page);
    ...
    qSort(lista_prowadzacych.begin(), lista_prowadzacych.end(),
    ...
    for (int i = 0; i < lista_prowadzacych.size() - 1; i++)
        if (lista_prowadzacych[i].nazwa() == lista_prowadzacych[i + 1].nazwa())
            lista_prowadzacych[i].setNazwa("2 " +
                                           lista_prowadzacych[i].nazwa());
    labelHead->setText("ZAKOŃCZONO POBIERANIE\nPOBRANO " +
                       QString::number(lista_prowadzacych.size()) +
                       " PROWADZĄCYCH");
...
int LoginWindow::PolwroStrony(QTextStream &stream)
{
    ...
    while (!stream.atEnd())
    {
        ...
        while ((pos1 = line.indexOf("topicdays=0&amp;start=", pos2)) >= 0)
        {
            ...
            if (pos2 >= 0)
                strona =
                    qMax(strona, line.mid(pos1 + 22, pos2 - pos1 - 22).toInt());
        }
    }
    if (!tematy)
        return -1;
