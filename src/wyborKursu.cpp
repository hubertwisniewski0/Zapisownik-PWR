...
bool Window::porownajGodziny(QString g1, QString g2) // g2 - filtr
{
    if (g1[1].digitValue() != g2[1].digitValue())
        return false;
    int P1 = g1[3].digitValue() * 600 + g1[4].digitValue() * 60 +
             g1[5].digitValue() * 10 + g1[6].digitValue();
    int K1 = g1[7].digitValue() * 600 + g1[8].digitValue() * 60 +
             g1[9].digitValue() * 10 + g1[10].digitValue();
    int P2 = g2[3].digitValue() * 600 + g2[4].digitValue() * 60 +
             g2[5].digitValue() * 10 + g2[6].digitValue();
    int K2 = g2[7].digitValue() * 600 + g2[8].digitValue() * 60 +
             g2[9].digitValue() * 10 + g2[10].digitValue();
...
void Window::odznaczWTablicy(int start, int len, int dzien, bool typ,
                             int tydzien)
{
    if (tydzien == 2)
    {
...
void Window::ZablokujNakladajace()
{
    for (int i = 0; i < tree->topLevelItemCount(); i++)
    {
        ...
        {
            auto child =
                static_cast<TreeWidgetItem *>(tree->topLevelItem(i)->child(j));
            QStringList data =
                child->kodTermin().split(";", QString::SkipEmptyParts);
            for (QString str : data)
            {
                ...
                if (!CzyPozycjaWolna(godziny.first, godziny.second,
                                     str[1].digitValue(),
                                     str[0].digitValue()) ||
                    !SprawdzPotok(child->text(7), parent->text(9), parent))
                {
        ...
        for (int j = 0; j < parent->childCount(); j++)
        {
            ...
            if (box->isChecked())
            {
                for (int k = 0; k < parent->childCount(); k++)
                {
                    ...
                    if (child2 != child)
                        static_cast<CheckBox *>(tree->itemWidget(child2, 0))
                            ->setEnabled(false);
                }
                break;
            }
...
bool Window::SprawdzPotok(const QString &potok, const QString &kodK,
...
void Window::Zaznaczono()
{
    ...
    if (box->isChecked())
    {
        for (QString str : data)
        {
            ...
            QPair<int, int> godziny = convertDate(str.mid(3, 8));
            QString czas =
                str.mid(3, 8).insert(2, ":").insert(5, " - ").insert(10, ":");
            QString text = parent->text(0);
            int pos = text.indexOf(" | ");
            if (pos < 1)
                pos = text.size();
            if (CzyPozycjaWolna(godziny.first, godziny.second,
                                str[1].digitValue(), str[0].digitValue()))
            {
                odznaczWTablicy(godziny.first, godziny.second,
                                str[1].digitValue(), true, str[0].digitValue());
                wybraneZajecia.push_back(
                    QPair<CheckBox *, QTreeWidgetItem *>(box, child));
                int tydzien = 0;
                if (str[0].digitValue() == 0)
                    ...
                    tydzien = 5;
                QLabel *prowadzacy =
                    static_cast<QLabel *>(tree->itemWidget(child, 2));
                drawCourse(str[1].digitValue(), tydzien, godziny.first,
                           godziny.second, child->text(1), text.mid(0, pos),
                           parent->background(0).color(),
                           parent->foreground(0).color(), parent->text(9), czas,
                           prowadzacy->text());
            }
        }
    }
    else
    {
        ...
        {
            QPair<int, int> godziny = convertDate(str.mid(3, 8));
            odznaczWTablicy(godziny.first, godziny.second, str[1].digitValue(),
                            false, str[0].digitValue());
            wybraneZajecia.removeOne(
                QPair<CheckBox *, QTreeWidgetItem *>(box, child));
            for (auto x : naSiatce)
            {
    ...
    for (int i = 0; i < tree->topLevelItemCount(); i++)
    {
        ...
        if (!dalej)
        {
            for (int j = 0; j < parent->childCount(); j++)
            {
                ...
                CheckBox *box =
                    static_cast<CheckBox *>(tree->itemWidget(child, 0));
                if (box->isEnabled() && !child->schowany() &&
                    !child->isHidden())
                    wolne[i].push_back(child->kurs());
            }
        }
    }
    qSort(wolne.begin(), wolne.end(),
          [](const QList<Kurs> &l1, const QList<Kurs> &l2)
