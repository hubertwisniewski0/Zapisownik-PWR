...
Window::Window(QWidget *parent) : QMainWindow(parent)
{
    ...
    tree->setHeaderLabels(QStringList() << "Zaznacz"
                                        << "Kod grupy"
                                        << "Prowadzący"
                                        << "Ocena"
                                        << "Ilość opinii"
    ...
    QAction *zapiszPlanTymczasowo =
        toolBar->addAction(QIcon("icons/save.png"), "Zapisz plan tymaczasowo");
    QAction *wczytajTymczasowyPlan =
        toolBar->addAction(QIcon("icons/load.png"), "Wczytaj tymczasowy plan");
    QAction *dolosujKursy =
        toolBar->addAction(QIcon("icons/random1.png"), "Dolosuj kursy");
    QAction *wylosujCalyPlan =
        toolBar->addAction(QIcon("icons/random2.png"), "Wylosuj cały plan");
    connect(zapiszPlanTymczasowo, &QAction::triggered, this,
            &Window::zapiszPlanTymczasowoAction);
    connect(wczytajTymczasowyPlan, &QAction::triggered, this,
            &Window::wczytajTymczasowyPlanAction);
    connect(dolosujKursy, &QAction::triggered, this, &Window::losujPlan);
    connect(wylosujCalyPlan, &QAction::triggered,
            [=]
            {
                odznaczZaznaczone();
                losujPlan();
    ...
    for (int i = 0; i < tree->topLevelItemCount(); i++)
    {
        ...
        for (int j = 0; j < parent->childCount(); j++)
        {
            ...
            if (filtrowanie->ukryjZajete())
            {
                ...
                if (child->background(0).color().name() == "#ff0000" &&
                    !box->isChecked())
                {
            ...
            {
                if (filtrowanie->ukryjPelneW() &&
                    (child->text(6).toFloat()) <= 0)
                {
                    child->setHidden(true);
                    continue;
                }
            ...
            {
                QStringList oceny =
                    child->text(3).split("|", QString::SkipEmptyParts);
                float srednia = 0;
    ...
    else
    {
        for (int i = 0; i < parent->childCount(); i++)
        {
            ...
            if (box->isChecked())
            {
                ...
                {
                    auto child2 =
                        static_cast<TreeWidgetItem *>(parent->child(k));
...
{
    qSort(kursy.begin(), kursy.end(),
          [](const Kurs &s1, const Kurs &s2)
...
void Window::przypisz_prowadzacego(QList<Kurs> &lista)
{
    ...
    futureWatcher.setFuture(
        QtConcurrent::map(lista, [=](Kurs &data) { dopasuj(data, prow); }));
    dialog.exec();
