...
void BlokWidget::resizeFont()
{
    ...
    for (;; ++fontSizeGuess)
    {
        ...
        const QRect fontRect = QFontMetrics(testFont).boundingRect(
            rect(), Qt::TextWordWrap, baseText);
        if (fontRect.height() + 15 > baseRect.height() ||
            fontRect.width() + zapas > baseRect.width())
            break;
    }
    for (; fontSizeGuess > 1; --fontSizeGuess)
    {
        ...
        const QRect fontRect = QFontMetrics(testFont).boundingRect(
            rect(), Qt::TextWordWrap, baseText);
        if (fontRect.height() <= baseRect.height() &&
            fontRect.width() <= baseRect.width())
            break;
    }
...
void BlokWidget::setKodGrupy(const QString kg) { kod_grupy = kg; }

void BlokWidget::setKodKursu(const QString kk) { kod_kursu = kk; }

QString BlokWidget::kodGrupy() const { return kod_grupy; }

QString BlokWidget::kodKursu() const { return kod_kursu; }
...
void BlokWidget::enterEvent(QEvent *)
{
    ...
    duzy->setText(text());
    duzy->show();
    const QRect fontRect = QFontMetrics(duzy->font())
                               .boundingRect(rect(), Qt::TextWordWrap, text());
    duzy->setGeometry(0, 0, 250, fontRect.height());
}

void BlokWidget::leaveEvent(QEvent *) { delete duzy; }

Day::Day(QString text, QWidget *parent) : QLabel(text, parent) {}

void Day::enterEvent(QEvent *) { lower(); }
