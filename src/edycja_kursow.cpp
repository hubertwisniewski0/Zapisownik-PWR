                                       ...
                                       const QList<Prowadzacy> &lista1)
{
    ...
    aktualne->setHorizontalHeaderLabels(QStringList() << "Kod kursu"
                                                      << "Kod grupy"
                                                      << "Nazwa"
                                                      << "Forma zajęć"
                                                      << "Prowadzący"
    ...
    connect(dodaj, &QPushButton::clicked, this,
            &EdycjaKursowDialog::dodajButton);
...
void DodajKurs::accepted(const QString &terminK)
{
    ...
    if (polwro->currentIndex() >= 0)
        ...
        {
            dodany.dane().push_back(
                dodany.lista().at(0).at(polwro->currentIndex()));
            dodany.prowadzacy()[0] = dodany.dane()[0].nazwa();
        }
    QDialog::accept();
}

Kurs DodajKurs::nowyKurs() const { return dodany; }
