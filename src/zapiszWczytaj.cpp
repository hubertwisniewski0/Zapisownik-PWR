...
void Kolizje(bool &all, int &val, Kurs &a, Kurs tmp, bool &kolizja)
{
    ...
    {
        QMessageBox msg;
        msg.setStandardButtons(QMessageBox::No | QMessageBox::NoToAll |
...
void Window::zapiszPlan()
{
    QString fileName = QFileDialog::getSaveFileName(
        this, "Zapisz plan", QDir::currentPath(), "Pliki planu (*.pln)");
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        msg.critical(
            this, "Błąd",
            QString("Nie można otworzyć pliku:\n%1").arg(file.fileName()));
        return;
    }
...
void Window::wczytajPlan()
{
    auto fileName = QFileDialog::getOpenFileName(
        this, "Wybierz plan do wczytania", QDir::currentPath(),
        "Pliki planu (*.pln)");
    if (fileName.size() == 0)
        return;
    QFile file(fileName);
...
{
    auto fileName = QFileDialog::getOpenFileName(
        this, "Wybierz plan kolegi do wczytania", QDir::currentPath(),
        "Pliki planu (*.pln)");
    if (fileName.size() == 0)
        return;
    QFile file(fileName);
...
void Window::wczytajProwadzacychZPliku()
{
    ...
    auto fileName = QFileDialog::getOpenFileName(
        this, "Wybierz plik z prowadzącymi do wczytania", QDir::currentPath(),
        "Pliki prowadzących (*.prw)");
    if (fileName.size() == 0)
        return;
    QFile file(fileName);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text))
    ...
    if (temp.size() != 0)
    {
        ...
        if (przedmioty.size() != 0)
        {
            ...
            int ret = msg.question(
                this, "Informacja",
                QString("Wczytano: %1 prowadzących.\nDopasować do kursów?")
                    .arg(temp.size()));
...
void Window::wczytajKursyZPliku()
{
    ...
    auto files = QFileDialog::getOpenFileNames(
        this, "Wybierz pliki z kursami do wczytania", QDir::currentPath(),
        "Pliki kursów (*.krs)");
    for (auto x : files)
    {
        if (x.size() == 0)
        ...
        else
        {
            ...
            else
            {
                ...
                while (!in.atEnd())
                {
                    ...
                    tmp.setProwadzacy(
                        in.readLine().split("||", QString::SkipEmptyParts));
    ...
    if (load)
    {
        if (prow.size() != 0 && temp.size() != 0)
        {
            ...
            int ret = msg.question(
                this, "Informacja",
                QString(
                    "Wczytano: %1 kursów.\nDopasować prowadzących z polwro?")
                    .arg(temp.size()));
            if (ret == QMessageBox::Yes)
...
void Window::wczytajKursyZEdukacji()
{
    ...
    else if (ret)
    {
        ...
        // folderPliku = "/kursy/" + fDane + "/" + l.folderZapisy().replace('/',
        // '-') + "/" + l.folderKryterium().replace('/', '-')
        // 	+ "/" + l.folderPlanStudiow().replace('/', '-') + "/" +
        // l.folderSemestr().replace('/', '-') + "/";
        // if(znajdzNumery(folderPliku, pos))
        // 	folderPliku.remove(pos, 6);
        // nazwaPliku = QString::number(l.lista_kursow.size()) + "|" +
...
{
    QString fileName = QFileDialog::getSaveFileName(
        this, "Zapisz plik", QDir::currentPath(), "Pliki kursów (*.krs)");
    if (fileName.size() == 0)
        return;
...
{
    QString fileName = QFileDialog::getSaveFileName(
        this, "Zapisz plik", QDir::currentPath(), "Pliki prowadzących (*.prw)");
    if (fileName.size() == 0)
        return;
