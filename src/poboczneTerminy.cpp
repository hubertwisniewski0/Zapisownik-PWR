...
QString terminy(QString text, QString &ret)
{
    ...
    do
    {
        ...
        if ((pos1 = text.indexOf("pn")) >= 0)
            ...
            kod += "1";
        else if ((pos1 = text.indexOf("sr")) >= 0 ||
                 (pos1 = text.indexOf("śr")) >= 0)
            kod += "2";
...
bool znajdzNumery(const QString &line, int &pos1)
{
    ...
    {
        if (line[i].digitValue() != -1 && line[i + 1].digitValue() != -1 &&
            line[i + 2].digitValue() != -1 && line[i + 3].digitValue() != -1 &&
            line[i + 4].digitValue() != -1 && line[i + 5].digitValue() != -1)
        {
            pos1 = i;
            return true;
        }
