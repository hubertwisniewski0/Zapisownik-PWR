...
void LoginWindow::GUI_AKZ()
{
    ...
    connect(jezyki, &QCheckBox::stateChanged, this,
            ...
            {
                if (jezyki->isChecked())
                    boxAKZ->setTabEnabled(1, true);
                else
                {
                    boxAKZ->setTabEnabled(1, false);
                    boxAKZ->setCurrentIndex(0);
                }
            });
    connect(wf, &QCheckBox::stateChanged, this,
            ...
            {
                if (wf->isChecked())
                    boxAKZ->setTabEnabled(2, true);
                else
                {
                    boxAKZ->setTabEnabled(2, false);
                    boxAKZ->setCurrentIndex(0);
                }
            });
    ...
    connect(download, &QPushButton::clicked, this, &LoginWindow::AKZ_download);
}
...
void LoginWindow::AKZ_download()
{
    ...
    QNetworkReply *reply =
        get_page("http://www.akz.pwr.edu.pl/katalog_zap.html");
    if (error(reply))
    {
    ...
    while (reply->size() != 0)
    {
        ...
        if (line.indexOf("<tr class=\"gradeX\">") >= 0)
        {
            ...
            QStringList listaProw =
                tymczas.split("<br/>", QString::SkipEmptyParts);
    ...
    labelHead->setText("ZAKOŃCZONO POBIERANIE\nPOBRANO " +
                       QString::number(lista_kursow.size() - stan1) +
                       " KURSÓW");
    ...
    connect(buttonMajor, &QPushButton::clicked, [=] { QDialog::done(2); });
}
