...
void LoginWindow::AcceptLogin()
{
    grey_out("LOGOWANIE...");
    QNetworkReply *reply =
        get_page("https://edukacja.pwr.wroc.pl/EdukacjaWeb/studia.do");
    if (error(reply))
    {
    ...
    reply =
        post(postData, "https://edukacja.pwr.wroc.pl/EdukacjaWeb/logInUser.do");
...
{
    QString zapisy = "https://edukacja.pwr.wroc.pl/EdukacjaWeb/"
                     "zapisy.do?clEduWebSESSIONTOKEN=" +
...
void LoginWindow::GUI_wybor_kierunku(QTextStream &data)
{
    ...
    connect(buttonMajor, &QPushButton::clicked, this,
            &LoginWindow::SelectMajor);
}
...
void LoginWindow::SelectMajor()
{
    ...
    postData.addQueryItem("cl.edu.web.TOKEN", clEduWebTOKEN);
    postData.addQueryItem("ineSluId",
                          ineSluId.value(comboKierunki->currentText()));
    postData.addQueryItem("event_WyborSluchaczaSubmit", "Wybierz");
    QNetworkReply *reply =
        post(postData, "https://edukacja.pwr.wroc.pl/EdukacjaWeb/zapisy.do");
    if (error(reply))
    {
        change_head_error("Błąd połączenia");
        return;
...
void LoginWindow::SelectSemester()
{
    ...
    grey_out("ŁĄCZENIE...");
    QString zapisy =
        "https://edukacja.pwr.wroc.pl/EdukacjaWeb/"
        "zapisy.do?clEduWebSESSIONTOKEN=" +
        clEduWebSESSIONTOKEN + "&event=wyborSemestruRow&rowId=" +
        id_semesters[comboKierunki->currentIndex()] +
...
void LoginWindow::SelectZapisy()
{
    ...
    else if (page.indexOf("Parametry wyszukiwania", 0) == -1)
    {
        ...
        // '-') + "/"; dropboxDialog d(db, folderPliku, this); d.show(); int ret
        // = d.exec(); if(ret)
        // {
...
void LoginWindow::Wektor_Plan_wybor_GUI()
{
    ...
    buttonMajor->disconnect();
    connect(buttonMajor, &QPushButton::clicked, this,
            &LoginWindow::select_Wektor_Plan);
}
...
QMap<QString, QString> LoginWindow::parse_uczppsid(QTextStream &stream,
                                                   const QString &name,
        ...
        if (start && !end)
            if ((pos1 = line.indexOf("option value=\"") + 14) >= 0)
                if ((pos2 = line.indexOf(end_name, pos1)) >= 0)
                    map.insert(stream.readLine().simplified(),
                               line.mid(pos1, pos2 - pos1).simplified());
    }
    return map;
}
...
{
    QString Kryterium[4] = {"Z_PLANU", "Z_WEKTORA_ZAP", "KURSY_POWTORKOWE",
                            "KURSY_ZALEGLE"};
    ...
    postData.addQueryItem("clEduWebSESSIONTOKEN", clEduWebSESSIONTOKEN);
    postData.addQueryItem("cl.edu.web.TOKEN", clEduWebTOKEN);
    postData.addQueryItem("KryteriumFiltrowania",
                          Kryterium[comboKierunki->currentIndex() - 1]);
    QNetworkReply *reply = post(
        postData, "https://edukacja.pwr.wroc.pl/EdukacjaWeb/"
                  "zapisy.do?event=ZapiszFiltr&event=wyborKryterium&href=");
    if (error(reply))
    {
        change_head_error("Błąd połączenia");
        return;
...
void LoginWindow::Select_PlanStudiow_GUI()
{
    ...
    buttonMajor->disconnect();
    connect(buttonMajor, &QPushButton::clicked, this,
            &LoginWindow::Select_PlanStudiow);
}
...
void LoginWindow::Select_PlanStudiow()
{
    ...
    postData.addQueryItem("UczPpsId",
                          mapUczpps.value(comboKierunki->currentText()));
    postData.addQueryItem("UczSppId", mapSemestrId.first());
    QNetworkReply *reply =
        post(postData, "https://edukacja.pwr.wroc.pl/EdukacjaWeb/"
                       "zapisy.do?event=ZapiszFiltr&event=wyborPlanu&href=");
...
void LoginWindow::SelectSemestr_GUI(const bool &nr)
{
    ...
    if (nr)
        connect(buttonMajor, &QPushButton::clicked, this,
                &LoginWindow::SelectSemestr_ZPLANU);
    else
        connect(buttonMajor, &QPushButton::clicked, this,
                &LoginWindow::SelectSemestr_ZWEKTORA);
}
...
void LoginWindow::SelectSemestr_ZPLANU()
{
    ...
    postData.addQueryItem("UczPpsId",
                          mapUczpps.value(comboKierunki->currentText()));
    postData.addQueryItem("UczSppId",
                          mapSemestrId.value(comboSemestr->currentText()));
    QNetworkReply *reply =
        post(postData, "https://edukacja.pwr.wroc.pl/EdukacjaWeb/"
                       "zapisy.do?event=ZapiszFiltr&event=wyborSemestru&href=");
...
void LoginWindow::SelectSemestr_ZWEKTORA()
{
    ...
    postData.addQueryItem("NrSemestru",
                          mapSemestrId.value(comboSemestr->currentText()));
    QNetworkReply *reply = post(
        postData, "https://edukacja.pwr.wroc.pl/EdukacjaWeb/"
                  "zapisy.do?event=ZapiszFiltr&event=wyborNrSemestru&href=");
...
bool LoginWindow::pytanieOWyborZrodla()
{
    ...
    // fDane.insert(fDane.indexOf("PO"), "/");
    // folderPliku = "/kursy/" + fDane + "/" + folderZapisy().replace('/', '-')
    // + "/" + folderKryterium().replace('/', '-')
    //   + "/" + folderPlanStudiow().replace('/', '-') + "/" +
    //   folderSemestr().replace('/', '-') + "/";
    ...
    //   QString czasNowego =
    //   info.modified().toTimeSpec(Qt::LocalTime).toString("dd.MM.yyyy hh.mm");
    //   QString pytanie = (QString("Na serwerze znajduje się plik z dnia
    ...
    // 	      aaa.setProwadzacy(stream.readLine().split("||",
    // QString::SkipEmptyParts)); 	      aaa.setTermin(stream.readLine());
...
void LoginWindow::ZnajdzBlokiGrupyKursy(QTextStream &stream,
                                        const QString &nazwa, const bool &nr)
{
    ...
    for (int j = 0; j < strony * 10; j += 10)
    {
        if (!nr)
        {
            ...
            QNetworkReply *reply = post(
                postData,
                "https://edukacja.pwr.wroc.pl/EdukacjaWeb/zapisy.do?href=");
...
void LoginWindow::zakonczonoPobieranie()
{
    ...
    buttonMajor->disconnect();
    connect(buttonMajor, &QPushButton::clicked, this, &QDialog::accept);
}
...
void LoginWindow::rozdajBlokiGrupyKursy(const bool &nr)
{
    ...
    else
    {
        for (auto a : lista_zajec)
        {
            labelHead->setText("POBIERAM: " + a.kod() + " - " +
                               a.rodzaj().simplified().right(1));
            QString link = "https://edukacja.pwr.wroc.pl" + a.link();
            QNetworkReply *reply = get_page(link);
            QByteArray page = reply->readAll();
            QTextStream stream(&page);
                        ...
                        const QString &kod)
{
    ...
    QString pagingIterName =
        "GrupyZajecioweKursu" + plan_wektor + kod + "ROViewIterator";
...
{
    tmp.setTermin("");
    QStringList terminys = parseClassBiala(stream, "</table>")
                               .split("</td>", QString::SkipEmptyParts);
    ...
    while (!stream.atEnd())
    {
        ...
        if (start)
        {
            if (line.indexOf("BIALA") >= 0)
            {
                ...
                {
                case 0:
                    tmp.setKodGrupy(usun_tagi(parseClassBiala(stream, "</td>"))
                                        .simplified());
                    break;
                case 1:
                    tmp.setKodKursu(usun_tagi(parseClassBiala(stream, "</td>"))
                                        .simplified());
                    break;
                case 2:
                    tmp.setNazwa(usun_tagi(parseClassBiala(stream, "</td>"))
                                     .simplified());
                    break;
                case 3:
                {
                ...
                case 7:
                    ...
                    break;
                case 8:
                    tmp.prowadzacy().push_back(
                        usun_tagi(parseClassBiala(stream, "</td>"))
                            .simplified());
                    break;
                case 9:
                    tmp.setForma(usun_tagi(parseClassBiala(stream, "</td>"))
                                     .simplified());
                    break;
                case 10:
                    parseClassBiala(stream, "</td>");
                ...
                case 12:
                    ...
                    break;
                case 13:
                    tmp.setPotok(usun_tagi(parseClassBiala(stream, "</td>"))
                                     .simplified());
                    break;
                case 14:
...
QString LoginWindow::get_token(const QString &token, const QString &end_token,
                               QNetworkReply *reply)
{
    QString data;
    int pos = 0, pos1 = 0;
...
void LoginWindow::grey_out(const QString &text)
{
    ...
    labelHead->setStyleSheet(
        QString("QLabel {color: %1; font-size: 18px;}")
