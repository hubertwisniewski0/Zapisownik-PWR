...
void Window::ustawMenu()
{
    ...
    connect(
        oProgramie, &QAction::triggered, this,
        [=]
        {
            QMessageBox msg;
            QString text =
                "<p style=\"text-align: center;\"><span "
                "style=\"font-size:20px;\"><strong>ZAPISOWNIK PWR "
                "0.9</strong></span></p> <p>Program do szybkiego układania "
                "planu dla studentów Politechniki Wrocławskiej, współpracujący "
                "z serwisem <em>edukacja.cl.</em></p> <p style=\"text-align: "
                "center;\">Autor projektu: Dawid Zaraza</p> <p "
                "style=\"text-align: center;\">Strona projektu: <a "
                "href=\"https://github.com/Diegos962/Zapisownik-PWR\">GITHUB</"
                "a></p> <br> W programie mogą występować błędy i nie zwalnia "
                "on użytkownika z myślenia. Przed zapisami sprawdź, czy "
                "ułożony przez ciebie plan faktycznie istnieje w serwisie "
                "edukacja.cl. Wszelkie błędy i uwagi proszę zgłaszać na adres: "
                "diegozaraza@wp.pl<p style=\"text-align: center;\"><span "
                "style=\"font-size:14px;\"><strong>Licencja</strong></span></"
                "p> Niniejszy program jest wolnym oprogramowaniem; można go "
                "rozprowadzać dalej i/lub modyfikować na warunkach Powszechnej "
                "Licencji Publicznej GNU, wydanej przez Fundację Wolnego "
                "Oprogramowania ‒ według wersji drugiej tej Licencji lub "
                "którejś z późniejszych wersji. Niniejszy program "
                "rozpowszechniany jest z nadzieją, iż będzie on użyteczny - "
                "jednak BEZ JAKIEJKOLWIEK GWARANCJI, nawet domyślnej gwarancji "
                "PRZYDATNOŚCI HANDLOWEJ albo PRZYDATNOŚCI DO OKREŚLONYCH "
                "ZASTOSOWAŃ. W celu uzyskania bliższych informacji należy "
                "zapoznać się z Powszechną Licencją Publiczną GNU. <br><p "
                "style=\"text-align: center;\">Copyright &copy; 2017 Dawid "
                "Zaraza</p>";
...
QMenu *Window::ustawMenuPlik()
{
    ...
    QAction *wczytajWybranyPlanKolegi =
        new QAction("Wczytaj plan kolegi", this);
    ...
    KursyZDropboxa->setEnabled(
        false); // Najprawdopodobniej dropbox przestal
    ...
    connect(zapiszProwadzacych, &QAction::triggered, this,
            &Window::ZapiszProwadzacych);
    connect(KursyZPliku, &QAction::triggered, this,
            &Window::wczytajKursyZPliku);
    connect(KursyZEdukacji, &QAction::triggered, this,
            &Window::wczytajKursyZEdukacji);
    connect(KursyZAKZ, &QAction::triggered, this, &Window::wczytajKursyZAKZ);
    // connect(KursyZDropboxa, &QAction::triggered, this,
    // &Window::PrzejrzyjDropboxa);
    connect(ProwadzacyZPliku, &QAction::triggered, this,
            &Window::wczytajProwadzacychZPliku);
    connect(ProwadzacyZPolwro, &QAction::triggered, this,
            &Window::wczytajProwadzacychZPolwro);
    connect(wczytajWybranyPlan, &QAction::triggered, this,
            &Window::wczytajPlan);
    connect(wczytajWybranyPlanKolegi, &QAction::triggered, this,
            &Window::wczytajPlanKolegi);
...
QMenu *Window::ustawMenuEdycja()
{
    ...
    connect(OdznaczZaznaczone, &QAction::triggered, this,
            &Window::odznaczZaznaczone);
...
QMenu *Window::ustawMenuWidok()
{
    ...
    QAction *pokazOPinieProwadzacego =
        new QAction("Ilość opinii prowadzącego", this);
    ...
    QObject::connect(zmienKolory, &QAction::triggered, this,
                     &Window::ZmienKolory);
    QObject::connect(
        pokazKodGrupy, &QAction::triggered, this,
        [=] { tree->setColumnHidden(1, !pokazKodGrupy->isChecked()); });
    QObject::connect(
        pokazProwadzacego, &QAction::triggered, this,
        [=] { tree->setColumnHidden(2, !pokazProwadzacego->isChecked()); });
    QObject::connect(
        pokazOceneProwadzacego, &QAction::triggered, this,
        [=]
        { tree->setColumnHidden(3, !pokazOceneProwadzacego->isChecked()); });
    QObject::connect(
        pokazOPinieProwadzacego, &QAction::triggered, this,
        [=]
        { tree->setColumnHidden(4, !pokazOPinieProwadzacego->isChecked()); });
    QObject::connect(pokazTermin, &QAction::triggered, this,
                     [=]
                     { tree->setColumnHidden(5, !pokazTermin->isChecked()); });
    QObject::connect(pokazMiejsca, &QAction::triggered, this,
                     [=]
                     { tree->setColumnHidden(6, !pokazMiejsca->isChecked()); });
    QObject::connect(pokazPotok, &QAction::triggered, this,
                     [=]
                     { tree->setColumnHidden(7, !pokazPotok->isChecked()); });
    QObject::connect(
        pokazLokalizacje, &QAction::triggered, this,
        [=] { tree->setColumnHidden(8, !pokazLokalizacje->isChecked()); });
    QObject::connect(ZmienWidok, &QAction::triggered, this,
                     &Window::PrzelaczWidok);
