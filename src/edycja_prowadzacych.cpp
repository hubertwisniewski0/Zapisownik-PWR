...
EdycjaProwadzacychDialog::EdycjaProwadzacychDialog(
    const QList<QList<Kurs>> &lista, const QList<Prowadzacy> &lista1)
{
    resize(1200, 500);
    setModal(true);
    ...
    table->setHorizontalHeaderLabels(QStringList() << "Imię i nazwisko"
                                                   << "Dopasowanie z polwro"
                                                   << "Ocena"
                                                   << "Ilość opinii"
                                                   << "Link");
    ...
    connect(zapisz, &QPushButton::clicked, this,
            &EdycjaProwadzacychDialog::zapiszClicked);
}
...
void EdycjaProwadzacychDialog::zapiszClicked()
{
    for (auto &b : kursy)
        for (auto &a : b)
            ...
            {
                auto x =
                    table->findItems(a.prowadzacy().at(i), Qt::MatchExactly);
                int row = table->row(x.at(0));
                a.dane()[i].setNazwa(
                    (static_cast<QComboBox *>(table->cellWidget(row, 1)))
                        ->currentText());
        ...
        for (auto x : y)
            for (int j = 0; j < x.prowadzacy().size(); j++)
                if (!czyNaLiscie(x.prowadzacy().at(j), temp))
                {
                    ...
                    temp.push_back(
                        QPair<QString, Kurs>(x.prowadzacy().at(j), x));
                    Prowadzacy xxx;
                    if (x.dane().size() > j)
                        xxx = x.dane().at(j);

                    QTableWidgetItem *tmp =
                        new QTableWidgetItem(x.prowadzacy().at(j));
                    QTableWidgetItem *tmp2 = new QTableWidgetItem(xxx.ocena());
                    QTableWidgetItem *tmp3 =
                        new QTableWidgetItem(xxx.iloscOpinii());
                    ...
                    if (p)
                        combo->addItem("");
                    int selected =
                        combo->findText(xxx.nazwa(), Qt::MatchExactly);
                    if (selected >= 0)
